...
def main(cfg: DictConfig):
    ...
    import time
    samples_dir = os.path.join(save_dir, f"multiclass_conditional_{time.strftime('%Y%m%d_%H%M%S')}")    
    os.makedirs(samples_dir, exist_ok=True)

    if cfg.conditional_score == "dps":
        save_path = f"dps_{cfg.sampling.euler_steps}_{cfg.sampling.clip_each_step}"
    elif cfg.conditional_score == "iwae": # iwae type!
    ...
    with open(os.path.join(samples_dir, "accuracy.txt"), "w") as f:
        ...
        f.write(f"Weights to sample: {ws.detach().cpu().tolist()}\n")
