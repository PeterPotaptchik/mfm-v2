...
@hydra.main(config_path="../conf/sample/", config_name="config_class_openai_ess.yaml", version_base="1.3")
def main(cfg: DictConfig):
    ...
    for i in tqdm.tqdm(range(0, labels.shape[0], cfg.sampling.batch_size), desc="Sampling"):
        ...
        def joint_score_fn(x, t):
            uncond_score = unconditional_score_fn(x, t)
            cond_score, ess, ess_rel, log_liks, grad_norms = conditional_score_fn(x, t, label_batch, return_ess=True)
            return uncond_score + cond_score, ess, ess_rel, log_liks, grad_norms

        samples_batch, ess, ess_rel, log_liks, grad_norms = sampler.sample((label_batch.shape[0], *datamodule.dims),
                                                                        joint_score_fn,
                                                                        ...
                                                                        return_ess=True)
        ess = ess.unsqueeze(0)
        ess_rel = ess_rel.unsqueeze(0)
        log_liks = log_liks.unsqueeze(0)
        grad_norms = grad_norms.unsqueeze(0)
        ess_list.append(ess)
        ess_rel_list.append(ess_rel)
        samples.append(samples_batch)
        log_liks_list.append(log_liks)
    ...
    ess_list = torch.cat(ess_list, dim=0)
    ess_rel_list = torch.cat(ess_rel_list, dim=0)
    log_liks_list = torch.cat(log_liks_list, dim=0)
    grad_norms_list = torch.cat(grad_norms_list, dim=0)
    print("log_liks_shape:", log_liks_list.shape)
    print("grad_norms_shape:", grad_norms_list.shape)
    ...
    samples_dir = os.path.join(save_dir, f"class_conditional_{time_str}_ess")
    ...
    torch.save(ess_list, os.path.join(samples_dir, "ess.pt"))
    torch.save(ess_rel_list, os.path.join(samples_dir, "ess_rel.pt"))
