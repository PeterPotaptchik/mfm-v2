...
#SBATCH --output=/n/netscratch/albergo_lab/Lab/ppotaptchik/distributional-mf/outputs/cifar/cifar_train-%A.out
#SBATCH --error=/n/netscratch/albergo_lab/Lab/ppotaptchik/distributional-mf/outputs/cifar/cifar_train-%A.err
...
srun --kill-on-bad-exit=1 --ntasks=1 torchrun --nproc_per_node="$SLURM_GPUS_PER_NODE" \
  ...
  scripts/train.py \
  wandb.name="cifar_long" \
  hydra.run.dir="$PROJECT_SCRATCH/outputs/cifar/\${now:%Y-%m-%d}/\${now:%H-%M-%S}"
