...
for euler_step in "${euler_steps[@]}"; do
  for iwae in "${n_iwaes[@]}"; do
    bs=$((256 / iwae))
    job_commands+=("python scripts/sample_openai_class.py ++conditional_score=iwae ++samples_per_label=2048 ++iwae.n_samples=$iwae ++sampling.batch_size=${bs} ++sampling.euler_steps=${euler_step}")
  done
done
