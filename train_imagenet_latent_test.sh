...
export MODEL_CHECKPOINT="/n/netscratch/albergo_lab/Lab/ppotaptchik/distributional-mf/outputs/2025-11-23/02-10-20/checkpoints/periodic-epoch=00-step=15000.ckpt"
...
srun bash -c '
  ...
  torchrun \
    ...
    scripts/train.py --config-name config_test \
    ...
    ++sampling.every_n_steps=2500 \
    ++use_parametrization=False \
    ++trainer.checkpoint_every_n_steps=2500 \
    ++loss.repa_weight=0.4 \
    ++weight_decay=0.05 \
